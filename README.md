...
a、npm run build:nosm 或者 gulp build:nosm ---    不压缩图片，启动打包后的项目，不生成sourceMap

b、npm run build:jk 或者 gulp buildJK ---  不压缩图片，不启动打包后的项目  【Jenkins上部署时采用】

b、npm run build:jknosm 或者 gulp buildJK:nosm ---  不压缩图片，不启动打包后的项目，不生成sourceMap  【Jenkins上部署时采用】
